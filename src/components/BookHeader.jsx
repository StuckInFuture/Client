...
function BookHeader({ scrollY, book }) {
  ...
  return (
    <Animated.View style={anims.header}>
      <Animated.Image blurRadius={15} style={anims.bg} source={{ uri: book.cover }} />

      <Animated.View style={anims.cover}>
        <SharedElement id={book.bookId}>
          <View style={styles.imgBox}>
            <Image style={styles.bookImg} source={{ uri: book.cover }} />
          </View>
        </SharedElement>
      </Animated.View>

      <Animated.View style={anims.title}>
        <Text bold center size={21} numberOfLines={2}>{book.name}</Text>
