...
function BookListScreen({ navigation }) {
  ...
  const onLayout = () => {
    loaded.value = withTiming(3, { duration: dark ? 300 : 600 });
  ...
  useEffect(() => {
    axios.get('https://bsuir-books-server.herokuapp.com/library', { headers: {"Authorization" : `Bearer ${myContext.token}` } }
    )
      .then((resp) => {
  ...
  const completed = allBooks.filter((b) => b.status === 'Will read');
  const wishlist = allBooks.filter((b) => b.status === 'Read');
  ...
  return (
    <Animated.View onLayout={onLayout} style={styles.screen}>
      ...
      >
        ...
        <BookList books={allBooks.filter((b) => b.status === 'Reading')} title="Reading" />
