...
function BookSearchScreen({ navigation }) {
  ...
  useEffect(() => {
    if (query.length > 0) {
      axios.get('https://bsuir-books-server.herokuapp.com/book', { headers: {"Authorization" : `Bearer ${myContext.token}` } }
        )
        .then((resp) => {
          const bks = resp.data.map((book) => ({
            ...book,
  ...
  return (
    <View onLayout={onLayout} style={styles.screen}>
      ...
      <Button style={styles.addButton} onPress={() => navigation.push('AddBook')}>
        <AntDesign size={21} name="book" style={styles.addIcon} />
      </Button>
      <Button style={styles.addButton2} title="author" onPress={() => navigation.push('AddAuthor')}>
        <AntDesign size={21} name="user" style={styles.addIcon} />
      </Button>
    </View>
  );
}
